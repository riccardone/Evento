...
namespace Evento
{
    ...
    /// </summary>
    public interface IDomainRepositoryAsync
    {     
        /// <summary>
        ...
        Task<TResult> GetByIdAsync<TResult>(string correlationId) where TResult : IAggregate, new();
        ...
        /// <returns>The Aggregate retrieved from the store in it's current state (all previous events applied)</returns>
        Task<TResult> GetByIdAsync<TResult>(string correlationId, int eventsToLoad) where TResult : IAggregate, new();
        ...
        Task DeleteAggregateAsync<TAggregate>(string correlationId, bool hard);
    }
}